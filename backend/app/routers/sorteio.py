...
def _buscar_jogadores(db: Session, jogador_ids: list[str]) -> list[Jogador]:
    jogadores = db.query(Jogador).filter(Jogador.id.in_(jogador_ids)).all()
...
def _recent_counts(db: Session, limite: int) -> dict[frozenset[str], int]:
    rounds = (
        db.query(Rodada)
...
def montar_grupos_route(dados: MontarGruposEntrada, db: Session = Depends(get_db)):
    ...
    jogadores = _buscar_jogadores(db, dados.jogador_ids)
...
def sortear_times_route(dados: SortearTimesEntrada, db: Session = Depends(get_db)):
    ...
    recent_counts = (
        _recent_counts(db, dados.limite_rodadas_recentes) if dados.considerar_historico else {}
    )
