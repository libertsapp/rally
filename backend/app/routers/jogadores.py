...
def criar_jogador(dados: JogadorEntrada, db: Session = Depends(get_db)):
    novo = Jogador(**dados.model_dump())
    db.add(novo)
    db.commit()
    db.refresh(novo)
    return novo
...
def atualizar_jogador(jogador_id: str, dados: JogadorEntrada, db: Session = Depends(get_db)):
...
def remover_jogador(jogador_id: str, db: Session = Depends(get_db)):
    jogador = db.query(Jogador).filter(Jogador.id == jogador_id).first()
    if not jogador:
        raise HTTPException(status_code=404, detail="Jogador não encontrado")
    db.delete(jogador)
...
def perfil_do_jogador(jogador_id: str, db: Session = Depends(get_db)):
    jogador = db.query(Jogador).filter(Jogador.id == jogador_id).first()
    if not jogador:
        raise HTTPException(status_code=404, detail="Jogador não encontrado")
    ...
    jogadores_por_id = {p.id: p for p in db.query(Jogador).all()}
    parceiros = [
